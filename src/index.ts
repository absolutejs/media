...
} from "./frameProcessorNoiseSuppressor";
export type {
  CreateFrameProcessorNoiseSuppressorOptions,
  CreateKrispNoiseSuppressorOptions,
  KrispFrameProcessor,
  NoiseSuppressionFrameProcessor,
} from "./frameProcessorNoiseSuppressor";
...
export type MediaFrameSource =
  "browser" | "provider" | "telephony" | "voice-runtime";
...
export type MediaFrameTransformPipeline = {
  push: (frame: MediaFrame) => Promise<readonly MediaFrame[]>;
  pushMany: (frames: readonly MediaFrame[]) => Promise<readonly MediaFrame[]>;
  transforms: readonly MediaFrameTransform[];
};

export type MediaProcessorNodeKind = "branch" | "filter" | "processor" | "sink";
...
export type MediaProcessorGraphState =
  "closed" | "draining" | "failed" | "idle" | "running";
...
export type MediaProcessorBackpressureEventKind =
  "complete" | "drop" | "queue" | "reject" | "start";
...
export type MediaProcessorNode = {
  ...
  process: (
    ...
    | readonly MediaFrame[]
    | undefined
    | Promise<boolean | MediaFrame | readonly MediaFrame[] | undefined>;
  flush?: () =>
    | MediaFrame
    | readonly MediaFrame[]
    | undefined
...
export type MediaProcessorGraph = {
  ...
  process: (frame: MediaFrame) => Promise<readonly MediaFrame[]>;
  processMany: (
    frames: readonly MediaFrame[],
...
export type MediaTransportState =
  "closed" | "closing" | "failed" | "idle" | "open";

export type MediaTransportEventKind =
  "backpressure" | "close" | "connect" | "error" | "frame-in" | "frame-out";
...
export type MediaTelephonyStreamEventKind =
  "connected" | "error" | "media" | "start" | "stop" | "unknown";
...
export type MediaWebRTCStatsReportInput = Omit<MediaWebRTCStatsInput, "stats"> &
...
export const createMediaFrame = (frame: MediaFrame): MediaFrame => frame;
...
export const parseTelephonyMediaFrame = (
  ...
  const carrier =
    input.carrier ?? firstString([envelope], ["provider"]) ?? "telephony";
...
export const parseTelephonyStreamEvent = (input: {
  ...
  const carrier =
    input.carrier ??
    firstString([envelope], ["provider", "carrier"]) ??
    "telephony";
  const frame =
    kind === "media"
      ? parseTelephonyMediaFrame({
...
export const buildMediaTransportReport = (input: {
  ...
  const failed =
    state === "failed" || events.some((event) => event.kind === "error");
...
export const createMediaTransport = (
  ...
  const frameHandlers = new Set<(frame: MediaFrame) => Promise<void> | void>();

  const record = (event: Omit<MediaTransportEvent, "at" | "state">) => {
...
export const createMediaFrameTransformPipeline = (
  input: {
    transforms?: readonly MediaFrameTransform[];
  } = {},
...
const normalizeProcessorResult = (
  frame: MediaFrame,
  result: boolean | MediaFrame | readonly MediaFrame[] | undefined,
): readonly MediaFrame[] => {
    ...
    process: async (frame) => {
      ...
      for (const route of selectedRoutes) {
        const routeOutput = normalizeProcessorResult(
          frame,
          route.process === undefined ? frame : await route.process(frame),
        );
        output.push(
          ...(annotateFrames
            ? routeOutput.map((routeFrame) =>
                annotateMediaProcessorBranchFrame(routeFrame, route, frame),
              )
            : routeOutput),
        );
      }
...
const getMediaProcessorFrameBranch = (
  frame: MediaFrame,
): string | undefined => {
  const branch =
    frame.metadata?.mediaBranch ?? frame.metadata?.mediaBranchRoute;
  return typeof branch === "string" ? branch : undefined;
};
...
export const createMediaProcessorFanIn = (input: {
  ...
  const emitGroup = async (
    ...
    return normalizeProcessorFlushResult(await join(joinInput)).map(
      (frame) => ({
        ...frame,
        metadata: {
          ...frame.metadata,
  ...
  return {
    edgeLabel: (_frame, output, outputFrame) => {
      ...
      return status === "complete" || status === "partial" ? status : "joined";
    },
    flush: async () => {
      if (!flushPartial) {
        return [];
    ...
    process: async (frame) => {
      ...
      const branch =
        input.branch?.(frame) ?? getMediaProcessorFrameBranch(frame);
...
export const createMediaProcessorGraph = (
  input: {
  ...
  const maxInFlightFrames = input.maxInFlightFrames ?? Number.POSITIVE_INFINITY;
  const maxNodeProcessingMs =
    input.maxNodeProcessingMs ?? Number.POSITIVE_INFINITY;
        ...
        if (output.length === 0) {
          ...
          for (const outputFrame of output) {
            edgeEvents.push({
              ...
              frameId: current.id,
              from: node.name,
              label: getProcessorEdgeLabel(node, current, output, outputFrame),
  ...
  const process = async (frame: MediaFrame) => {
    ...
    if (queuedTasks.length >= maxQueuedFrames) {
      if (queueOverflowStrategy === "reject") {
        pushBackpressureEvent("reject", frame);
        throw new Error("Media processor graph queue is full.");
      }
      pushBackpressureEvent("drop", frame);
      return [];
    }
...
export const buildMediaWebRTCStatsReport = (
  ...
  const inbound = stats.filter(
    (stat) =>
      stat.type === "inbound-rtp" && stringStat(stat, "kind") !== "video",
  );
  const outbound = stats.filter(
    (stat) =>
      stat.type === "outbound-rtp" && stringStat(stat, "kind") !== "video",
  );
...
export const buildMediaPipelineCalibrationReport = (
  ...
  const turnCommitFrames = frames.filter(
    (frame) => frame.kind === "turn-commit",
  );
  ...
  const jitterValues = frames
    ...
    .filter((value): value is number => value !== undefined);
  const jitterMs =
    jitterValues.length > 0 ? Math.max(...jitterValues) : undefined;
  const inputFormat =
    input.inputFormat ?? inputFrames.find((frame) => frame.format)?.format;
  const outputFormat =
...
export type MediaArtifactWriteInput<
  TSummary,
  TJson = unknown,
> = MediaArtifactPair<TSummary, TJson> & {
  dir: string;
  slug: string;
};
...
export const renderMediaProcessorGraphMarkdown = (
  ...
  const edgeRows = report.edges
    ...
    .join("\n");
  const issues = report.errors.map((event): MediaPipelineCalibrationIssue => ({
    code: event.kind,
    message:
      event.error ?? `Processor graph ${event.kind} (state ${event.state}).`,
    severity: "error",
  }));
  const lines = [
    `# ${title}`,
...
const truncateArrays = (
  ...
  if (Array.isArray(value)) {
    const head = value
      .slice(0, limit)
      .map((entry) => truncateArrays(entry, limit, seen));
